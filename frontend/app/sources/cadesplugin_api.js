;(function () {
    ...
    function cpcsp_console_log(level, msg) {
        //IE9 не может пиcать в конcоль еcли не открыта вкладка developer tools
    ...
    // Функция активации объектов КриптоПро Э­ЦП Browser plug-in
    function CreateObject(name) {
        ...
        if (isIE()) {
            ...
            if (name.match(/X509Enrollment/i)) {
                try {
                    // Объекты CertEnroll сздаются через CX509EnrollmentWebClassFactory
    ...
    // Функция активации асинхронных объектов КриптоПро Э­ЦП Browser plug-in
    ...
    function show_firefox_missing_extension_dialog() {
        if (!window.cadesplugin_skip_extension_install) {
            ...
            ovr.innerHTML = "<div id='cadesplugin_ovr_item' style='position:relative; width:400px; margin:100px auto; background-color:#fff; border:2px solid #000; padding:10px; text-align:center; opacity: 1; z-index: 1500'>" +
              ...
              "<p>Для работы КриптоПро ЭЦП Browser plugin на данном сайте необходимо расширение для браузера. \n" +
              "Убедитесь, что оно у Вас включено или установите его." +
    ...
    function install_opera_extension() {
        if (!window.cadesplugin_skip_extension_install) {
            document.addEventListener('DOMContentLoaded', function () {
                ...
                ovr.innerHTML = "<div id='cadesplugin_ovr_item' style='position:relative; width:400px; margin:100px auto; background-color:#fff; border:2px solid #000; padding:10px; text-align:center; opacity: 1; z-index: 1500'>" +
                  "<button id='cadesplugin_close_install' style='float: right; font-size: 10px; background: transparent; border: 1; margin: -5px'>X</button>" +
                  "<p>Для работы КриптоПро Э­ЦП Browser plugin на данном сайте необходимо установить расширение из каталога дополнений Opera." +
                  "<p><button id='cadesplugin_install' style='font:12px Arial'>Уcтановить раcширение</button></p>" +
                  "</div>";
                document.getElementsByTagName("Body")[0].appendChild(ovr);
    ...
    // Вспомогательна функция для NPAPI
    ...
    function check_npapi_plugin() {
        try {
            ...
            if (mimetype) {
                ...
                if (plugin) {
                    plugin_loaded_error("Плагин загружен, но не создаютя обьекты");
                } else {
    ...
    function check_plugin_working() {
        ...
        if (isIeLessThan9) {
            plugin_loaded_error("Internet Explorer версии 8 и ниже не поддерживаетя");
            return;
        }
